...
try:
    ...
    from netkiller.svg.elements import Text, Line, Circle, Rectangle, Group, Path
...
class Fishbone:
    ...
    def __scan(self):
        ...
        self.canvasWidth = self.fishtailWidth + countEffect / 2 * (
                self.causeWidth + self.gap + self.space) + self.fishheadWidth
        self.canvasHeight = (countCause + 1) * self.causeHeight * 2 + self.spineHeight * 2
        self.spineX = (countCause + 1) * self.causeHeight + self.causeHeight
        self.spineWidth = self.fishtailWidth + countEffect / 2 * (
                self.causeWidth + self.gap + self.space)
    ...
    text {
      ...
      # font-style: italic;
        ...
        for effect, cause in self.__fishbone['up'].items():
            ...
            textWidth = self.font.getTextSize(effect)
            group.append(Rectangle(self.canvasX, self.canvasY, textWidth, self.causeHeight, stroke="none", fill=color.random(), rx="5", ry="5"))

            self.canvasY += self.causeHeight
            group.append(Text(effect, self.canvasX + textWidth / 2, self.canvasY - self.causeHeight / 4, text_anchor="middle", fill="white"))

            self.gapLeft = self.canvasX + textWidth
            self.gapRight = self.gapLeft + self.gap
            group.append(Line(self.gapLeft, self.canvasY - self.causeHeight / 2, self.gapRight, self.spineX, stroke=color.random(), stroke_width="2"))

            self.canvasY += self.causeHeight / 2
            for item in cause:
                cx = self.gap * self.canvasY / (self.spineX - self.causeHeight / 2)
                textWidth = self.font.getTextSize(item)
                self.canvasX = self.gapLeft + cx - textWidth - 10
                self.canvasY += self.causeHeight
                group.append(Circle(cx=self.gapLeft + cx, cy=self.canvasY - 15, r="3", fill="red", stroke="none", stroke_width="1"))
                group.append(Text(item, self.canvasX, self.canvasY - self.causeHeight / 4, fill=color.random()))
        ...
        for effect, cause in self.__fishbone['down'].items():
            ...
            textWidth = self.font.getTextSize(effect)
            group.append(Rectangle(self.canvasX, self.canvasY - self.causeHeight, textWidth, self.causeHeight, stroke="none", fill=color.random(), rx="5", ry="5"))
            # y += self.causeHeight
            group.append(Text(effect, self.canvasX + textWidth / 2, self.canvasY - self.causeHeight / 4, text_anchor="middle", fill="white"))

            self.gapLeft = self.canvasX + textWidth
            self.gapRight = self.gapLeft + self.gap
            group.append(Line(self.gapLeft, self.canvasY - 15, self.gapRight, self.spineX, stroke=color.random(), stroke_width="2"))

            # self.canvasX = self.gapRight
            # self.canvasX = self.fishtailWidth
            self.canvasY -= self.causeHeight / 2
            for item in cause:
                # cx = self.gap * self.canvasY / (self.spineX - self.causeHeight / 2)
                cx = self.gap * (self.canvasHeight - self.canvasY + 30) / (self.spineX - self.causeHeight / 2)
                textWidth = self.font.getTextSize(item)
                self.canvasX = self.gapLeft + cx - textWidth - 10
                self.canvasY -= self.causeHeight
                group.append(Circle(cx=self.gapLeft + cx, cy=self.canvasY - 15, r="3", fill="red", stroke="none", stroke_width="1"))

...
try:
    ...
    from netkiller.svg import elements as ele
    # from netkiller.svg.elements import Text, Line, Circle, Rectangle, Group, Path, Use
    from netkiller.svg.color import Color
    from netkiller.svg.font import Font
    from netkiller.svg import defs
...
class Canvas:
    ...
    def getTextSize(self, text):
        ...
        font = ImageFont.truetype(self.fontFamily, size=18, encoding="utf-8")
        # except IOError:
...
class Calendar(Canvas):
    ...
    def __table(self, top):
        group = ele.Group(id="table")
        # 画布最左边
        # group.append(draw.Line(1, top, 1, self.canvasHeight, stroke='black'))
        left = 5

        group.append(ele.Text("任务", font_size=20, x=left, y=top + 20 + self.rowHeight * 2, fill="#555555"))
        left += self.nameTextSize + 5
        group.append(
            ele.Line(left, top + self.rowHeight * 2, left, self.canvasHeight, stroke="grey"))
        group.append(ele.Text("开始日期", font_size=20, x=left + 5, y=top + 20 + self.rowHeight * 2, fill="#555555"))
        left += self.dateTextSize + 20
        group.append(
            ele.Line(left, top + self.rowHeight * 2, left, self.canvasHeight, stroke=self.lineColor))
        left += 5
        group.append(
            ele.Text("截止日期", font_size=20, x=left, y=top + 20 + self.rowHeight * 2,
                     fill="#555555"))
        left += self.dateTextSize + 15
        group.append(
            ele.Line(left, top + self.rowHeight * 2, left, self.canvasHeight,
                     stroke="grey"))
        left += 5
        group.append(ele.Text("工时", font_size=20, x=left, y=top + 20 + self.rowHeight * 2, fill="#555555"))
        left += 45
        group.append(
            ele.Line(left, top + self.rowHeight * 2, left, self.canvasHeight,
                     stroke=self.lineColor))
        left += 5
        group.append(ele.Text("资源", font_size=20, x=left, y=top + 20 + self.rowHeight * 2, fill="#555555"))
    ...
    def __weekdays(self, top, begin, end):
        ...
        for day in range(beginDay, endDay + 1):
            ...
            r = ele.Rectangle(x, top + self.rowHeight * 2, self.columeWidth, self.calendarHeight - self.rowHeight * 2, fill=color).append(ele.Title(str(day)))
            weekGroups[weekNumberOfYear].append(r)
            # 周分割线
            if weekday == 6:
                weekGroups[
                    weekNumberOfYear].append(ele.Line(x + self.columeWidth, top + self.rowHeight, x + self.columeWidth, self.canvasHeight,
                                                      stroke="black"))
            # 日期
            weekGroups[
    ...
    def __month(self, top, months):
        ...
        for begin, end in months:
            ...
            monthGroups[month] = ele.Group(id="month" + str(month))
    ...
    def calendarYear(self, top):
        ...
        for year, month in years.items():
            ...
            yearGroups[year] = ele.Group(id="year" + str(year))
    ...
    def calendarHeader(self):
        ...
        for key, value in self.calendarYear(self.canvasTop).items():
            header.append(value)

        header.append(ele.Line(self.canvasLeft, self.canvasTop + self.rowHeight, self.canvasWidth, self.canvasTop + self.rowHeight, stroke="grey"))

        header.append(ele.Line(self.canvasLeft, self.canvasTop + self.rowHeight * 2, self.canvasWidth, self.canvasTop + self.rowHeight * 2,
                               stroke="grey"))
    ...
    def calendar(self):
        ...
        #     background.append(value)
        # 月线
        background.append(ele.Line(left, top + self.rowHeight, self.canvasWidth, top + self.rowHeight, stroke="grey"))
        ...
        # 周线
        background.append(ele.Line(self.canvasLeft, top + self.rowHeight * 2, self.canvasWidth, top + self.rowHeight * 2, stroke="grey"))
        # 日期线
        ...
        top = top + self.rowHeight * 3 + self.splitLineHeight * 3
        # 分割线
        ...
        # 日历边框
        background.append(ele.Rectangle(self.canvasLeft, self.canvasTop, self.canvasWidth, self.calendarHeight, fill="none", stroke="black"))
...
class Gantt(Calendar, Canvas):
    ...
    def __legend(self):
        ...
        top = 10
        self.draw.append(
            ele.Text("https://www.netkiller.cn - design by netkiller", font_size=12, x=self.canvasWidth - 250,
                     y=self.height - self.rowHeight / 2,
        ...
        self.draw.append(ele.Image(8, 8, 100, 34.99, license, embed=True))
    ...
    def __items(self, line, subitem=False):
        ...
        if self.showTable:
            ...
            table.append(ele.Text(
                line["name"], font_size=self.fontSize, x=10 + (self.textIndent * self.textIndentSize), y=top + 23,
                text_anchor="start"))
            # text.append(draw.TSpan(line['begin'], text_anchor='start'))
            # text.append(draw.TSpan(line['end'], text_anchor='start'))

            table.append(ele.Text(
                line["start"], font_size=self.fontSize, x=self.nameTextSize + 15, y=top + 23, text_anchor="start"))
            table.append(
                ele.Text(
                    line["finish"], font_size=self.fontSize, x=self.nameTextSize + self.dateTextSize + 35, y=top + 23,
                    text_anchor="start"))
            # if 'progress' in line:
        ...
        if subitem:
            ...
            if "milestone" in line and line["milestone"]:
                ...
                group.append(
                    ele.Text(datetime.strptime(
                        line["start"], "%Y-%m-%d").strftime("%Y年%m月%d日"), font_size=16, x=left + 30,
                             y=top + 23, text_anchor="start", fill="#333333"))
            else:
                # 工时
                r = ele.Rectangle(left + 1, top + 8, right - 1, self.barHeight, fill="#00C7C1", stroke="grey")
                ...
                if "progress" in line and line["progress"] > 0:
                    ...
                    progressBar = ele.Rectangle(left + 2, top + 11, 30 * progress - 2, self.progressHeight,
                                                fill="#B2F2EA")
                    # progressBar.append_title(str(progress))
                    group.append(progressBar)
                    group.append(
                        ele.Text("%d%%" % ((progress / (
                                end + 1)) * 100), font_size=10, x=left + 5, y=top + 22, text_anchor="start",
    ...
    def link(self, fromTask, toTask):
        ...
        linkGroup = ele.Group(id="link")
        x = fromTask["x"] + fromTask["width"] + 1
        y = fromTask["y"] + 15
        # arrow = defs.Marker(-0.1, -0.51, 0.9, 0.5, scale=4, orient="auto")
        arrow = defs.Marker(id="arrow", scale=4, orient="auto")
        arrow.append(ele.Polyline((-0.1, 0.5), (-0.1, -0.5), (0.9, 0), fill="black", close=True))
        path = ele.Path(stroke="black", stroke_width=2, fill="none", marker_end="arrow")
    ...
    def rander(self):
        ...
        self.canvasHeight = self.canvasTop + (
                self.rowHeight * 3 + self.splitLineHeight * 3) + self.rowHeight * self.lineNumber + self.splitLineHeight * (
                                    self.lineNumber - 1)
        self.width = self.canvasWidth + 2
        self.height = self.canvasHeight + 2 + self.rowHeight * 2
        # self.calendarHeight = self.canvasHeight
        self.calendarHeight = self.canvasHeight - self.canvasTop
  ...
  /* font-size: 16px;*/
        ...
        if self.__title:
            group = ele.Group(id="title", onclick="this.style.stroke = 'green'; ")
            group.append(ele.Text(x=self.canvasWidth / 2, y=25, font_size=30, text_anchor="middle", font_family=self.fontFamily).append(self.__title))
            if self.name:
                group.append(ele.Text(self.name, font_szie=16, x=5, y=self.height - self.rowHeight / 2))
        ...
        self.canvasTop = self.canvasTop + self.rowHeight * 3

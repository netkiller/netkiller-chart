...
def parse_heading(line, current_level, current_node, root, parent_map):
    """解析标题行，返回更新后的节点状态"""
...
def parse_list_item(line, current_node, parent_map, list_stack):
    """解析列表项行，返回更新后的节点状态"""
...
def parse_markdown(markdown_text):
    ...
    for line in lines:
        ...
        # 先尝试解析为标题
        current_heading_level, current_node, parent_map = parse_heading(
            line, current_heading_level, current_node, root, parent_map
        )

        # 如果不是标题，尝试解析为列表项
        if line.startswith((' ', '\t', '-', '*', '+')):
            current_node, parent_map, list_stack = parse_list_item(
